        ...
        if self.num_encoder_layers > 0:
            for i, enc_ind in enumerate(self.use_encoder_idx):
                ...
                if self.training or self.eval_spatial_size is None:
                    ...
                    # Fast path: use the cached eval-size positional embedding.
                    # Robustness path: if the actual feature map differs from
                    # eval_spatial_size (profiling, alternate inference size,
                    # etc.), rebuild it for the current HxW instead of failing
                    # with a token-count mismatch.
