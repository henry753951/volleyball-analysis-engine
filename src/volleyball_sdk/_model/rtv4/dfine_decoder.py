    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if self.training or i == self.eval_idx:
                ...
                # Preserve the query state at every supervised decoder layer.
                # The final state powers inference; selected earlier states can
                # receive low-weight semantic auxiliary supervision in training.
        ...
        if not dec_out_query_features:
            raise RuntimeError("Decoder did not produce query features.")
        return (
            torch.stack(dec_out_bboxes),
            torch.stack(dec_out_logits),
            torch.stack(dec_out_query_features),
...
class DFINETransformer(nn.Module):
    __share__ = ["num_classes", "num_actions", "num_group_classes", "eval_spatial_size"]
    __inject__ = ["multitask_head"]
    ...
    def __init__(
        ...
        self.reg_max = reg_max
        self.multitask_head = multitask_head
        self.semantic_aux_layers = max(0, int(semantic_aux_layers))
        self.semantic_aux_refine_pose = bool(semantic_aux_refine_pose)
        if self.semantic_aux_layers > max(0, self.num_layers - 1):
            raise ValueError("semantic_aux_layers cannot exceed num_layers - 1")
        ...
        # A modular multitask head is injected by the enhanced YAML.  The
        # original linear/MLP heads remain available when loading an old config,
        # which keeps the project backward compatible for controlled ablations.
        if (
            self.num_actions > 0
            or self.num_group_classes > 0
            or self.num_human_keypoints > 0
            or self.num_court_keypoints > 0
        ) and self.eval_idx != num_layers - 1:
            raise ValueError(
                "Multitask heads require eval_idx to be the final decoder layer (use eval_idx=-1)."
            )
        if self.multitask_head is None:
            self.dec_action_head = (
                nn.Linear(hidden_dim, self.num_actions) if self.num_actions > 0 else None
            )
            self.dec_group_head = (
                nn.Linear(hidden_dim, self.num_group_classes)
                if self.num_group_classes > 0
                else None
            )
            if self.num_human_keypoints > 0 or self.num_court_keypoints > 0:
                self.pose_trunk = nn.Sequential(
                    nn.LayerNorm(hidden_dim),
                    nn.Linear(hidden_dim, self.pose_hidden_dim),
                    nn.SiLU(inplace=True),
                    nn.Linear(self.pose_hidden_dim, self.pose_hidden_dim),
                    nn.SiLU(inplace=True),
                )
            else:
                self.pose_trunk = None
            self.human_pose_head = (
                nn.Linear(self.pose_hidden_dim, self.num_human_keypoints * 2)
                if self.num_human_keypoints > 0
                else None
            )
            self.court_pose_head = (
                nn.Linear(self.pose_hidden_dim, self.num_court_keypoints * 2)
                if self.num_court_keypoints > 0
                else None
            )
        else:
            self.dec_action_head = None
            self.dec_group_head = None
            self.pose_trunk = None
            self.human_pose_head = None
            self.court_pose_head = None
        ...
        # Cache anchors for the configured evaluation size. Dynamic-size
        # inference regenerates them only when the token count differs.
    ...
    def _reset_parameters(self, feat_channels):
        ...
        for head in (
            ...
            if head is not None:
                init.normal_(head.weight, std=0.01)
                init.constant_(head.bias, 0)
        if self.pose_trunk is not None:
            for module in self.pose_trunk:
    ...
    def _get_decoder_input(
        ...
        if self.training or self.eval_spatial_size is None:
            ...
            # Cached anchors are generated for eval_spatial_size.  If an eval
            # call uses another resolution, regenerate anchors for the actual
            # feature shapes instead of multiplying/gathering mismatched token
            # counts.  Normal configured-size inference still uses the cache.
            expected_tokens = sum(int(h) * int(w) for h, w in spatial_shapes)
            if self.anchors.shape[1] != expected_tokens:
                anchors, valid_mask = self._generate_anchors(spatial_shapes, device=memory.device)
            else:
    ...
    def forward(self, feats, targets=None, center_feature=None):
        ...
        final_query_features = out_query_features[-1]
        ...
        # Every enabled branch remains fully differentiable through the final
        # decoder queries, projected image features and predicted boxes.
        if self.multitask_head is not None:
            ...
            if self.dec_action_head is not None:
                out["pred_actions"] = self.dec_action_head(final_query_features)
            if self.dec_group_head is not None:
                out["pred_group_logits"] = self.dec_group_head(final_query_features)
            if self.pose_trunk is not None:
                pose_features = self.pose_trunk(final_query_features)
                if self.human_pose_head is not None:
                    out["pred_human_keypoints"] = torch.sigmoid(
                        self.human_pose_head(pose_features)
                    ).reshape(
                        final_query_features.shape[0],
                        final_query_features.shape[1],
                        self.num_human_keypoints,
                        2,
                    )
                if self.court_pose_head is not None:
                    out["pred_court_keypoints"] = torch.sigmoid(
                        self.court_pose_head(pose_features)
                    ).reshape(
                        final_query_features.shape[0],
                        final_query_features.shape[1],
        ...
        if (
            ...
            and self.semantic_aux_layers > 0
            and out_query_features.shape[0] > 1
        ):
            start_layer = max(0, out_query_features.shape[0] - 1 - self.semantic_aux_layers)
            semantic_aux_outputs = []
            for layer_index in range(start_layer, out_query_features.shape[0] - 1):
                semantic = {
                    "pred_logits": out_logits[layer_index],
                    "pred_boxes": out_bboxes[layer_index],
                }
                semantic.update(
                    self.multitask_head(
                        out_query_features[layer_index],
                        out_logits[layer_index],
                        out_bboxes[layer_index],
                        projected_features,
                        center_feature=center_feature,
                        refine_pose=self.semantic_aux_refine_pose,
                        include_court=False,
                    )
                )
                # Without feature refinement the head's coarse and final pose
                # tensors are identical.  Do not count the same intermediate
                # prediction twice under two loss names.
                if not self.semantic_aux_refine_pose:
                    semantic.pop("pred_human_keypoints_coarse", None)
                    semantic.pop("pred_court_keypoints_coarse", None)
                semantic_aux_outputs.append(semantic)
            out["semantic_aux_outputs"] = semantic_aux_outputs
